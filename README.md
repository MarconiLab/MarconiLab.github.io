...
## 6) Change Index.html to the following code
...
var channelKeys =[];
channelKeys.push({channelNumber:8, name:'SmartCitizen',key:'1IFRRK4GWC6RCDQJ',
   fieldList:[{field:1,axis:'T'},{field:2,axis:'T'},{field:3,axis:'T'},{field:4,axis:'T'},{field:5,axis:'T'},{field:6,axis:'T'},{field:7,axis:'T'},{field:8,axis:'T'}]});
channelKeys.push({channelNumber:9, name:'SmartCitizen 2',key:'1VJW9UJMSR2QAQN7',
   fieldList:[{field:1,axis:'O'},{field:2,axis:'O'},{field:3,axis:'O'},{field:4,axis:'O'},{field:5,axis:'O'},{field:6,axis:'O'},{field:7,axis:'T'},{field:8,axis:'T'}]});
...
$(document).ready(function() 
{
 ...
 function loadThingSpeakChannel(sentChannelIndex,channelNumber,key,sentFieldList) {
   ...
   // get the Channel data with a webservice call
 	$.getJSON('http://84.88.85.11:3000/channels/'+channelNumber+'/feed.json?callback=?&amp;offset=0&amp;results=2500;key='+key, function(data) 
             ...
             if (document.getElementById("Update").checked)
             {
              ...
              {  
               (function(channelIndex)
               {
                ...
                $.getJSON('http://http://84.88.85.11:3000/channels/'+channelKeys[channelIndex].channelNumber+'/feed/last.json?callback=?&amp;offset=0&amp;location=false;key='+channelKeys[channelIndex].key, function(data) 
...
function loadChannelHistory(sentChannelIndex,channelNumber,key,sentFieldList,sentNumLoads,maxLoads) {
   ...
   // get the Channel data with a webservice call
 	...
 	$.getJSON('http://84.88.85.11:3000/channels/'+channelNumber+'/feed.json?callback=?&amp;offset=0&amp;start=2013-01-20T00:00:00;end='+end+';key='+key, function(data) 
